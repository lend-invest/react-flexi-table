...
var FlexiTable = (_temp = _class = function (_React$Component) {
  ...
  function FlexiTable(props) {
    ...
    _this.clearSizeStyles = function () {
      if (_this.tableStyle) {
        while (_this.tableStyle.sheet.cssRules.length) {
          _this.tableStyle.sheet.deleteRule(0);
        }
      }
    };
    ...
    _this.updateSizeStyles = function (calcColumnWidths) {
      ...
      _this.tableStyle.sheet.insertRule('.FlexiTable-' + _this.uuid + ' .FlexiTableRow { min-width: ' + rowWidth + 'px; max-width: ' + rowWidth + 'px; width: ' + rowWidth + 'px;}', 0);
      columnsWithWidths.forEach(function (c) {
        _this.tableStyle.sheet.insertRule('.FlexiTable-' + _this.uuid + ' .FlexiTable--column-' + c.name + ' .FlexiTableCell { min-width: ' + c.width + 'px; max-width: ' + c.width + 'px; width: ' + c.width + 'px;}', 0);
      });
    };
  ...
  FlexiTable.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    if (this.props.columns !== nextProps.columns || this.props.halfGutterWidth !== nextProps.halfGutterWidth || this.props.marginWidth !== nextProps.marginWidth) {
